...
public class beanDB {
	...
	public void conectarBD() throws SQLException {
		try {
			...
			cn = DriverManager.getConnection("jdbc:mysql://altair1.altair.edu.es/HR", "csi1", "altair123$%");
	...
	public String[] resConsultaSelect(String selec, int numcol) throws SQLException {
		...
		try {
			// Este statement lo creamos así­ porque necesitamos que el result
			// sea scrollable
			stmt = cn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY);
			rs = stmt.executeQuery(selec);
			boolean hayFilas = rs.last(); // Movemos a la ultima fila para
			...
			if (hayFilas) {
				...
				while (rs.next()) {
					...
					while (j < numberOfColumns) {
						try {
							if (!rs.getString(j + 1).equals("null"))
	...
	// -- Se diferencia del anterior en que no hace la conexiï¿½n ni la
	// desconexiï¿½n --
